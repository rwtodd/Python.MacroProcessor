...
@ Let's define a more complex command module, this time for tracking and referencing end-notes for
an HTML document.  Note that, this time, we have to maintain state across two commands, so we put them in the same object.  We register bound methods with the processor in |init_processor_module()|, which works great for our purposes.
...
@ An example of using this module might look like:
